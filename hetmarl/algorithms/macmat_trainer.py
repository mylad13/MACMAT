...
from hetmarl.utils.util import get_grad_norm, huber_loss, mse_loss
...
class MACMAT_Trainer:
    """
    Trainer class for MACMAT to update policies.
    ...
    def __init__(self,
        ...
        self.initial_entropy_coef = args.entropy_coef
        self.use_linear_entropy_decay = args.use_linear_entropy_decay
        ...
        self.use_centralized_training = args.use_centralized_training
        ...
        if not self.use_linear_entropy_decay:
            self.entropy_coef = self.initial_entropy_coef
    
    def entropy_decay(self, episode, episodes):
        """
        Decay the entropy coefficient linearly.
        :param episode: (int) current training episode.
        :param episodes: (int) total number of training episodes.
        """
        decay_start_episode = 1000
        if episode < decay_start_episode:
            self.entropy_coef = self.initial_entropy_coef
        else:
            self.entropy_coef = self.initial_entropy_coef - (self.initial_entropy_coef * ((episode-decay_start_episode) / float(episodes-decay_start_episode)))
    ...
    def ppo_update(self, sample): #TODO: make sure active_masks are used correctly to mask out info after envs are done
        ...
        if not self.use_centralized_training: #unused
        ...
        elif self.use_centralized_training:
            old_action_log_probs_batch = check(old_action_log_probs_batch).to(**self.tpdv)
            adv_targ = check(adv_targ).to(**self.tpdv)
            value_preds_batch = check(value_preds_batch).to(**self.tpdv)
            return_batch = check(return_batch).to(**self.tpdv)
    ...
    def async_ppo_update(self, sample):
        ...
        if self.use_centralized_training:
            ...
            if 'dict' in obs_batch.__class__.__name__:
                for key in obs_batch.keys():
                    if obs_batch[key].dtype == np.object_:
                        continue
                    share_obs_batch[key] = check(share_obs_batch[key]).to(**self.tpdv)
    ...
    def train(self, buffer):
        ...
        train_info['learning_rate'] = self.policy.optimizer.param_groups[0]['lr']
