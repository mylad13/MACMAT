...
def multidiscrete_autoregreesive_act(decoder, obs_rep, obs, batch_size, n_agent, action_dim, tpdv,
    ...
    for i in range(n_agent):
        ...
        for logit in logits:
            logit_i = logit[:, i, :]
            # print("shape of logit_i in auto_regressive_act is ", logit_i.shape)
            if available_actions is not None:
            ...
            distri = Categorical(logits=logit_i)
                # print("distri: ", distri)
            action = distri.probs.argmax(dim=-1) if deterministic else distri.sample()
            # print("action shape is: ", action.shape) # (B,)
            action_log = distri.log_prob(action)
        ...
        if i + 1 < n_agent:
            ...
            for act in out_action:
                ...
                cnt2 += 1
        # print("output_action_log shape is: ", output_action_log.shape) # (B, n_agent, n_action)
        # print("cnt2 is: ", cnt2)
        # print("shifted_action: ", shifted_action[3,1,:,:])
...
def multidiscrete_parallel_act(decoder, obs_rep, obs, action, batch_size, n_agent, action_dim, tpdv,
    ...
    # shifted_action = torch.zeros((batch_size, n_agent, action_dim[0] + 1, action_dim[1] + 1)).to(**tpdv)
    # shifted_action[:, 0, 0, 0] = 1
    # print("shape of active agents: ", active_agents.shape) # (batch, n_agent, 1)

    shifted_action = torch.zeros((batch_size, n_agent, 1 + action_dim[0] + action_dim[1])).to(**tpdv)
    shifted_action[:, 0, 0] = 1
    # print("available actions of time 0 and agent 1 are: ", available_actions[4,1])
    cnt2 = 0
    for act in action: # iterates in the batch
        # print("act shape is: ", act.shape) # (n_agent, n_action)
        act = act.cpu().numpy()
    ...
    cnt1 = 0
    for logit in logits:
        # print("shape of logit in parallel_act is ", logit.shape)
        if available_actions is not None:
            # print("going through action masking")
            if cnt1 == 0: #for the row action
        ...
        # print("shape of action log is: ", action_log.shape)
        action_logs.append(action_log)
        entropy = distri.entropy().unsqueeze(-1)
        # print("action log is ", action_log[10:20,:,:])
        # print("entropy is ", entropy[10:20,:,:])
        # print("active agents are ", active_agents[10:20,:,:])
        if active_agents is not None:
    ...
    # print("action_logs after cat: ", action_logs.shape)
