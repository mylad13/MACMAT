...
class NCP_No_Motor(NCP):
    """ Inter-command hierarchy without motor neurons."""
    ...
    def __init__(self,
        ...
        super().__init__(
            ...
            motor_neurons=0,  # No motor neurons
            sensory_fanout=sensory_fanout,
            inter_fanout=inter_fanout,
            recurrent_command_synapses=recurrent_command_synapses,
            motor_fanin=0,  # No motor neurons, so no fanin
            seed=seed
        )

    @property
    def num_layers(self):
        return 2        # Only inter and command layers
    ...
    def get_type_of_neuron(self, neuron_id):
        if neuron_id < self._num_command_neurons:
            return "command"
        return "inter"
    
    # skip the command-to-motor layer build
    def _build_command__to_motor_layer(self):
        pass