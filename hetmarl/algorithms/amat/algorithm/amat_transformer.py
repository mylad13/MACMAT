...
class Encoder(nn.Module):
    ...
    def __init__(self, state_dim, obs_dim, action_dim, n_block, n_embd, n_hidden, n_hidden_r, n_embd_vit, n_head, n_agent,
                  n_agent_types, max_num_targets, spawn_hazards, max_num_hazards, use_rnn, rnn_type, encode_state):
        ...
        global_out_channel1 = 64 # Old and new CATMiP
        ...
        hidden_size1 = 2*n_embd        # old CATMiP
        # hidden_size1 = n_embd        
        ...
        # Option 1: Concatenating of the encodings.
        # self.obs_encoder = nn.Sequential(
        #     init_(nn.Linear(local_flattened_size2+global_flattened_size1+n_embd, hidden_size1), activate=True), self.active_func2, nn.LayerNorm(hidden_size1),
        #     init_(nn.Linear(hidden_size1, n_embd), activate=True), self.active_func2)
        
        # Option 2: Adding the class encoding to obs embeddings
        self.obs_encoder = nn.Sequential(
        ...
        # # state encoder not used (was defined in old CATMiP)
    ...
    def forward(self, state, obs, active_agents=None):
        ...
        # Option 1: Concatenating of the encodings.
        # obs_encoding = torch.cat((global_obs_encoding, local_obs_encoding, agent_class_encoding), dim=1)
...
class Decoder(nn.Module):
    ...
    def __init__(self, obs_dim, action_dim, n_block, n_embd, n_embd_vit, n_head, n_agent, n_agent_types,
        ...
        if action_type == 'Discrete':
            ...
            # # self.agent_class_encoder = nn.Sequential(init_(nn.Linear(n_agent_types, n_embd), activate=True), self.active_func2)
            # # Option 1: concat agent class encoding and action embeddings
            # # self.action_encoder = nn.Sequential(
            # #     init_(nn.Linear(2*n_embd, n_embd), activate=True), self.gelu) # 2*n_embd for agent_class_encoding and action_embeddings
            ...
            self.agent_class_encoder = nn.Sequential(init_(nn.Linear(n_agent_types, n_embd), activate=True), self.active_func2)
            # probably don't need conv2d for the action encoder?
            self.heads = []
    ...
    def forward(self, action, obs_rep, obs, rnn_states=None):
        ...
        if self.action_type == 'Discrete':
            ...
            for block in self.blocks:
                x = block(x, obs_rep)
            logit = self.head(x)
        else: # Not used in this implementation
...
class AsynchronousMultiAgentTransformer(nn.Module):

    def __init__(self, state_dim, obs_dim, action_dim, n_agent, n_agent_types, max_num_targets, spawn_hazards, max_num_hazards,
                 n_block, n_embd, n_hidden, n_hidden_r, n_embd_vit, n_head, use_rnn=False, rnn_type='GRU', encode_state=False, device=torch.device("cpu"),
                 action_type='Discrete', dec_actor=False, share_actor=False):
        super(AsynchronousMultiAgentTransformer, self).__init__()
    ...
    def get_actions(self, state, obs, available_actions=None, deterministic=False, active_agents=None, rnn_states_actor=None, rnn_states_critic=None):
        ...
        # print("obs shape: ", obs['agent_class_identifier'].shape)
        v_loc, obs_rep = self.encoder(state, obs, active_agents)
