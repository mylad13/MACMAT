...
def update_linear_schedule(optimizer, epoch, total_num_epochs, initial_lr, initial_rnn_lr=None, warmup=None):
    """Warmup to initial lr/rnn_lr, then linearly decay."""
    if warmup is not None and epoch < warmup:
        lr = initial_lr * (epoch + 1) / warmup
        rnn_lr = initial_rnn_lr * (epoch + 1) / warmup if initial_rnn_lr is not None else None
    else:
        decay_epoch = epoch - warmup if warmup is not None else epoch
        decay_total = total_num_epochs - (warmup if warmup is not None else 0)
        lr = initial_lr - (initial_lr * (decay_epoch / float(decay_total)))
        rnn_lr = None
        if initial_rnn_lr is not None:
            rnn_lr = initial_rnn_lr - (initial_rnn_lr * (decay_epoch / float(decay_total)))
    for i, param_group in enumerate(optimizer.param_groups):
        if initial_rnn_lr is not None and i == 0:
            param_group['lr'] = rnn_lr
        else:
            param_group['lr'] = lr
        # print(f"Updating param group {i} with lr {param_group['lr']}")
