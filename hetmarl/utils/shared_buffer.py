...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, share_obs_space, act_space, env_name):
        ...
        if 'Dict' in obs_shape.__class__.__name__:
            ...
            for key in obs_shape:
                if key == "agent_prompt" or key == "other_agent_info" or obs_shape[key].__class__.__name__ == 'Sequence':
                    # Allocate an object array for text prompts or variable-length sequences.
                    self.obs[key] = np.empty((self.episode_length + 1, self.n_rollout_threads, num_agents), dtype=object)
                    self.all_obs[key] = np.empty((self.episode_length + 1, self.n_rollout_threads, num_agents), dtype=object)
                else:   
                    # If obs_shape[key].shape is None, use an empty tuple as fallback.
                    shape_ext = obs_shape[key].shape if (hasattr(obs_shape[key], "shape") and obs_shape[key].shape is not None) else ()
                    self.obs[key] = np.zeros(
                        (self.episode_length + 1, self.n_rollout_threads, num_agents, *shape_ext),
                        dtype=np.float32
                    )
                    self.all_obs[key] = np.zeros(
                        (self.episode_length + 1, self.n_rollout_threads, num_agents, *shape_ext),
                        dtype=np.float32
                    )
        ...
        if self.use_rnn:
            if self._rnn_uses_tuple_state:
                ...
                self.rnn_states_critic = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, self.recurrent_hidden_size), dtype=np.float32)
    ...
    def insert(self, share_obs, obs, rnn_states, rnn_states_critic, actions, action_log_probs,
        ...
        self.step = (self.step + 1) % self.episode_length # This is tau, the global macro step counter
    ...
    def async_insert(self, share_obs, obs, rnn_states, rnn_states_critic, actions, action_log_probs,
        ...
        for e, a, agent_step in active_agents:
            agent_step = int(agent_step - 1) # this is tau^i, the per-agent macro step counter
            self.masks[agent_step + 1, e, a] = masks[e, a].copy()

            for key in self.obs.keys():
                self.obs[key][agent_step + 1, e, a] = obs[key][e][a].copy()
                self.all_obs[key][self.step + 1, e] = obs[key][e].copy()
            ...
            if self.use_rnn:
                if self._rnn_uses_tuple_state:
                    ...
                    self.rnn_states_critic[1][agent_step + 1, e, a] = rnn_states_critic[1][e, a].copy()
                else:
            ...
            self.active_steps[agent_step + 1 , e, a] = (self.step + 1) * self.masks[agent_step + 1, e, a] # mapping from tau^i to tau (kappa)
            self.agent_steps[self.step + 1 , e, a] = (agent_step + 1 ) * self.masks[agent_step + 1, e, a] # mapping from tau to tau^i
            self.was_agent_active[self.step + 1 , e, a] = True * self.masks[agent_step + 1, e, a].astype(bool)
            self.update_step[e, a] = agent_step + 1 # this is T^i, the maximum number of per agent macro steps
            self.update_step_masks[agent_step , e, a] = 1

        self.agent_groups[self.step + 1] = agent_groups.copy()
        self.step = self.step + 1 #increment tau
        
    def update_mask(self, steps): #only called at the end of episode, before compute
        for e in range(self.n_rollout_threads):
            for a in range(self.num_agents):
                invalid_active_steps = self.active_steps[self.update_step[e,a],e,a]
    ...
    def after_update(self):
        ...
        if self.asynch:
            ...
            if self._mixed_obs:               
                ...
                for key in obs_shape:
                    if key == "agent_prompt" or key == "other_agent_info" or obs_shape[key].__class__.__name__ == 'Sequence':
                        # Allocate an object array for text prompts.
                        self.obs[key] = np.empty((self.episode_length + 1, self.n_rollout_threads, self.num_agents), dtype=object)
                        self.all_obs[key] = np.empty((self.episode_length + 1, self.n_rollout_threads, self.num_agents), dtype=object)
                    else:   
                        # If obs_shape[key].shape is None, use an empty tuple as fallback.
                        shape_ext = obs_shape[key].shape if (hasattr(obs_shape[key], "shape") and obs_shape[key].shape is not None) else ()
                        self.obs[key] = np.zeros(
                            (self.episode_length + 1, self.n_rollout_threads, self.num_agents, *shape_ext),
                            dtype=np.float32
                        )
                        self.all_obs[key] = np.zeros(
                            (self.episode_length + 1, self.n_rollout_threads, self.num_agents, *shape_ext),
            ...
            if self.use_rnn:
                if self._rnn_uses_tuple_state:
                    ...
                    # )
                    self.rnn_states_critic = (
                        np.zeros((self.episode_length + 1, self.n_rollout_threads, self.num_agents, self.recurrent_hidden_size), dtype=np.float32),
                        np.zeros((self.episode_length + 1, self.n_rollout_threads, self.num_agents, self.recurrent_hidden_size), dtype=np.float32)
                    )
                else:
                    ...
                    self.rnn_states_critic = np.zeros((self.episode_length + 1, self.n_rollout_threads, self.num_agents, self.recurrent_hidden_size), dtype=np.float32)
            self.value_preds = np.zeros(
    ...
    def async_compute_returns(self, next_values, value_normalizer=None):
        ...
        for e in range(self.n_rollout_threads):
            for a in range(self.num_agents):
                if self._use_gae:
                    self.value_preds[self.update_step[e,a], e, a] = next_values[e, a]
                else:
                    self.returns[self.update_step[e,a], e, a] = next_values[e, a]
        ...
        if self._use_gae:
            ...
            for step in reversed(range(final_step + 1)):
                if self._use_popart or self._use_valuenorm:
                    ...
                    # print(f"at step {step}, agents have returns {self.returns[step]} and reward {self.rewards[step]}")
                else:
    ...
    def async_feed_forward_generator_transformer(self, advantages, num_mini_batch=None, mini_batch_size=None):
            ...
            for i, index in enumerate(agents_active_step_indices):
                ...
                if self.use_rnn:
                    if self._rnn_uses_tuple_state:
                        ...
                        temp_rnn_states_critic.append((
                            squeezed_rnn_states_critic[0][list_of_active_agents[i][0], list_of_active_agents[i][1]],
                            squeezed_rnn_states_critic[1][list_of_active_agents[i][0], list_of_active_agents[i][1]]
                        ))
                    else:
        ...
        for indices in sampler:
            ...
            if self._mixed_obs:
                ...
                share_obs_batch = obs_batch.copy()
            ...
            if self.use_rnn:
                if self._rnn_uses_tuple_state:
                    ...
                    rnn_states_critic_batch = (
                        ...
                        reordered_rnn_states_critic[1][indices].reshape(-1, *reordered_rnn_states_critic[1].shape[2:])
                    )
                else:
