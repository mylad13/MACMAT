...
@hydra.main(config_path='../config', config_name='hydra_train_config', version_base="1.1")
def main(config):
    ...
    if all_args.use_wandb:
        if all_args.resume_run_with_id:
            run = wandb.init(config=all_args_dict,
                            ...
                            dir=str(run_dir),
                            job_type="training",
                            id=all_args.resume_run_with_id,
                            resume="must",
                            reinit=True)
        elif all_args.fork_run_with_id:
            run = wandb.init(config=all_args_dict,
                             project=all_args.env_name,
                            entity=all_args.wandb_name,
                            notes=socket.gethostname(),
                            ...
                            dir=str(run_dir),
                            job_type="training",
                            fork_from=f"{all_args.fork_run_with_id}?_step=???",
                            reinit=True)
